...
static bool app_room_settings_tick(bui_room_ctx_t *ctx, bui_room_t *room, uint32_t elapsed);
static void app_room_settings_button(bui_room_ctx_t *ctx, bui_room_t *room, bool left, bool right);
static void app_room_settings_draw(bui_room_ctx_t *ctx, const bui_room_t *room, bui_bitmap_128x32_t *buffer);

static uint8_t app_room_settings_elem_size(const bui_menu_menu_t *menu, uint8_t i);
static void app_room_settings_elem_draw(const bui_menu_menu_t *menu, uint8_t i, bui_bitmap_128x32_t *buffer, int16_t y);
...
static void app_room_settings_enter(bui_room_ctx_t *ctx, bui_room_t *room, bool up) {
	...
	bui_menu_init(&APP_ROOM_SETTINGS_ACTIVE.menu, 2, inactive.focus, true);
	app_disp_invalidate();
}
...
static bool app_room_settings_tick(bui_room_ctx_t *ctx, bui_room_t *room, uint32_t elapsed) {
	return bui_menu_animate(&APP_ROOM_SETTINGS_ACTIVE.menu, elapsed);
}
...
static void app_room_settings_draw(bui_room_ctx_t *ctx, const bui_room_t *room, bui_bitmap_128x32_t *buffer) {
	bui_menu_draw(&APP_ROOM_SETTINGS_ACTIVE.menu, buffer);
}
...
static void app_room_settings_elem_draw(const bui_menu_menu_t *menu, uint8_t i, bui_bitmap_128x32_t *buffer, int16_t y) {
	switch (i) {
	case 0:
		bui_font_draw_string(buffer, "Reset", 64, y + 2, BUI_DIR_TOP, bui_font_open_sans_extrabold_11);
		break;
	case 1:
