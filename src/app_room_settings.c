...
static void app_room_settings_handle_event(bui_room_ctx_t *ctx, const bui_room_event_t *event);

static void app_room_settings_enter(bool up);
static void app_room_settings_exit(bool up);
static void app_room_settings_time_elapsed(uint32_t elapsed);
static void app_room_settings_draw();
static void app_room_settings_button_clicked(bui_button_id_t button);
...
const bui_room_t app_rooms_settings = {
	.event_handler = app_room_settings_handle_event,
};
...
static void app_room_settings_enter(bool up) {
	...
	bui_room_alloc(&app_room_ctx, sizeof(app_room_settings_active_t));
...
static void app_room_settings_exit(bool up) {
	if (up) {
		...
		bui_room_dealloc(&app_room_ctx, sizeof(app_room_settings_active_t));
...
static void app_room_settings_draw() {
	bui_menu_draw(&APP_ROOM_SETTINGS_ACTIVE.menu, &app_bui_ctx);
}
...
static void app_room_settings_button_clicked(bui_button_id_t button) {
	...
	case BUI_BUTTON_NANOS_BOTH:
		...
		case 0:
			bui_room_enter(&app_room_ctx, &app_rooms_reset, NULL, 0);
			break;
		case 1:
...
static void app_room_settings_elem_draw(const bui_menu_menu_t *menu, uint8_t i, bui_ctx_t *bui_ctx, int16_t y) {
	switch (i) {
	case 0:
		bui_font_draw_string(&app_bui_ctx, "Reset", 64, y + 2, BUI_DIR_TOP, bui_font_open_sans_extrabold_11);
		break;
	case 1:
